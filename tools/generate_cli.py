...
def resolve_spec(name: str) -> Path:
    ...
    Specs live at ``openapi_specs/<service>/openapi.{yaml,json}``. That layout is now a
    property of the pinned ref in ``spec-source.yaml`` rather than of whichever checkout
    happens to be on disk, so the fallback that also accepted the older flat
    ``openapi_specs/<Service>.yaml`` naming is gone — no reachable ref still uses it.
    """
    candidates = [
...
def shared_enum_type(models_root: str, schema: dict) -> str | None:
    """Kiota's name for an enum the spec declares once and ``$ref``s.

    A parameter that inlines its ``enum`` gets a type generated beside the endpoint
    (see :func:`kiota_enum_type`); one that points at a named schema gets a single shared
    type under ``Models`` instead. Wellbore DDMS's ``orient`` is the second kind — it
    ``$ref``s ``JSONOrient`` — and using the per-endpoint name for it names a type that was
    never generated.
    """
    name = schema.get("$ref", "").rsplit("/", 1)[-1]
    return f"global::Equinor.OsduCsharpClient.{models_root}.Models.{name}" if name else None


def kiota_param_type(schema: dict, wrapped: bool) -> str:
    """The C# type Kiota gives a query parameter — which is not always what the spec says.

    Kiota does not see through ``anyOf: [{...}, {"type": "null"}]``, the FastAPI spelling of
    an optional parameter. Where that wrapper is present it falls back to a plain mapping:
    ``format: int64`` stops meaning ``long``, and an array of strings collapses to a single
    ``string``. Matching the spec instead of matching Kiota produces code that does not
    compile, so this deliberately reproduces the coarser result.
    """
    # A boolean parameter is a flag. `Option<bool?>` renders as `--describe <describe>` and
    # makes the user type `--describe true`, which no CLI does. `Option<bool>` defaults to
    # false and is sent unconditionally — identical to omitting it, since these parameters
    # default to false in the spec too. A parameter defaulting to true would not be safe to
    # treat this way, so it keeps the nullable form.
    if schema.get("type") == "boolean" and schema.get("default") is not True:
        return "bool"
    if not wrapped:
        return csharp_type(schema)
    if schema.get("type") == "array":
        return csharp_type(schema.get("items") or {"type": "string"})
    return csharp_type({k: v for k, v in schema.items() if k != "format"})
...
def normalise_schema(schema: dict, spec: dict) -> dict:
    """Reduce a parameter schema to the one that describes its value.

    Two wrappers hide the real schema, and both make `csharp_type` fall through to its
    ``string`` default — which then fails to compile against a Kiota query property typed
    ``int?`` or a generated enum. The compile error is at least loud; the same fallthrough on
    an enum would have compiled and silently dropped the allowed values.

    * ``anyOf: [{...}, {"type": "null"}]`` is how FastAPI spells an optional parameter, and
      the whole Wellbore DDMS spec is generated by FastAPI. Every optional query parameter on
      every bulk endpoint arrives this way.
    * ``$ref`` to a named schema, which is how a reusable enum is declared — Wellbore DDMS's
      ``orient`` points at ``JSONOrient`` rather than inlining ``[split, columns]``.

    Only an ``anyOf``/``oneOf`` with exactly one non-null branch is unwrapped. A genuine union
    is left alone: it has no single C# type, and quietly picking its first branch would be a
    guess dressed up as a derivation.
    """
    for _ in range(4):  # depth guard: a $ref can point at another wrapper
        ...
        # The wrapper may carry the description and default; the branch carries the type.
...
def build_service(manifest_path: Path) -> Service:
    ...
    # Error messages name the spec by its path within openapi_specs, which is the same
    # wherever that directory came from.
    ...
    service = Service(
        ...
        # path: the specs may be a fetched copy under openapi_specs/, a sibling checkout or
        # an OSDU_SPECS_DIR elsewhere. Embedding the path would make generated output differ
        # between checkouts and fail CI's `git diff --exit-code` staleness gate for no real
        # reason.
...
# Options added to the root with `Recursive = true` are in scope on every command, so a
...
def check_alias(flag: str, where: str, what: str) -> None:
    if flag in RESERVED_ALIASES:
        raise ManifestError(
...
def build_command(entry: dict, operation: dict, where: str, models_root: str,
    ...
    for name, cfg in (entry.get("params") or {}).items():
        ...
        check_alias(cfg["flag"], where, f"parameter {name!r} flag")
        if cfg.get("short"):
            check_alias(cfg["short"], where, f"parameter {name!r} short alias")
        ...
        # A $ref survives the nullable wrapper, so look for it on either.
        shared = shared_enum_type(models_root, raw_schema) or shared_enum_type(
            models_root, next((b for b in (raw_schema.get("anyOf")
                                           or raw_schema.get("oneOf") or [])
                               if b.get("$ref")), {}))
        ...
        params.append(Param(
            ...
            cs_type=kiota_param_type(schema, wrapped),
            ...
            enum_type=(shared or kiota_enum_type(models_root, entry["op"]["path"],
    ...
    # One group, or several: `[a, b]` and `[[a, b], [c, d]]` are both accepted, because a
    # command can have more than one pair of contradictory options and silently honouring
    # only the first would be worse than requiring the nesting.
    raw_exclusive = entry.get("mutually-exclusive") or []
    groups = raw_exclusive if raw_exclusive and isinstance(raw_exclusive[0], list) else (
        [raw_exclusive] if raw_exclusive else [])
    mutually_exclusive = [[str(name) for name in group] for group in groups]
    ...
    for group in mutually_exclusive:
        for name in group:
            if name not in selectable:
                raise ManifestError(
                    f"{where}: mutually-exclusive names {name!r}, which is neither a param "
                    f"nor a body field of this command: {sorted(selectable)}")
        if len(group) < 2:
            raise ManifestError(
                f"{where}: mutually-exclusive group {group} needs at least two names")
...
def option_declaration(flag: str, short: str | None, cs_type: str, help_text: str,
                       required: bool, var: str, allowed: list[str] | None = None, parts: int = 0) -> list[str]:
    ...
    if parts:
        ...
        # rather than reporting. Parsing here keeps `--bbox 49.1,7.7,48.8,8.0` working and
        # turns a bad value or a wrong count into a parse error the user can read.
        lines[-1] = lines[-1].rstrip("};") + "    CustomParser = result =>"
        lines += [
            "            {",
            "                var raw = string.Join(\",\", result.Tokens.Select(token => token.Value));",
            "                var supplied = raw.Split(',', StringSplitOptions.RemoveEmptyEntries);",
            f"                if (supplied.Length != {parts})",
            "                {",
            f"                    result.AddError({csharp_string(flag + f' needs exactly {parts} comma-separated numbers.')});",
            "                    return [];",
            "                }",
...
def emit_command(service: Service, command: Command) -> list[str]:
    ...
    for group in command.mutually_exclusive:
        ...
        chosen = [by_name[name] for name in group]
    ...
    if command.body:
        if command.body.from_fields:
            ...
            for bodyfield in command.body.fields:
                ...
                if bodyfield.parts:
                    # One flag carrying several coordinates, spread across the paths it
                    # names: `--bbox 58,2,62,6` is four numbers in four places. Typing
                    # four separate flags for a bounding box would be worse than the
                    # feature is worth.
